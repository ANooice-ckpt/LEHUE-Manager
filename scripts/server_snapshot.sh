...
case "$ACTION" in
  export)
    NAME="${ARGUMENT:-LEHUE_TEST_snapshot_$(date -u +%Y%m%d_%H%M%SZ).zip}"
    docker compose run --rm --no-deps api python scripts/test_snapshot.py export "/app/data/snapshots/$NAME"
    echo "Snapshot: server/data/snapshots/$NAME"
    ;;
  restore)
    ...
    KEY="$(printf '%s\n' "$RESULT" | tail -1 | python3 -c 'import json,sys; print(json.load(sys.stdin)["credential_encryption_key"])')"
    ROLLBACK="$(printf '%s\n' "$RESULT" | tail -1 | python3 -c 'import json,sys; print(json.load(sys.stdin)["rollback_snapshot"])')"
    temporary="$(mktemp)"
    awk -v value="$KEY" 'BEGIN{done=0} /^CREDENTIAL_ENCRYPTION_KEY=/{if(!done){print "CREDENTIAL_ENCRYPTION_KEY=" value;done=1};next}{print} END{if(!done)print "CREDENTIAL_ENCRYPTION_KEY=" value}' .env > "$temporary"
    mv "$temporary" .env; chmod 600 .env
    docker compose up -d api
    trap - ERR
    echo "TEST snapshot restored; previous state: $ROLLBACK"
    ;;
