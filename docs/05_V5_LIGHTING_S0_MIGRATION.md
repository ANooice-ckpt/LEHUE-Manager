...
- 明确“不愿意/不同意/拒绝”的答卷不进入候选池。
- 合并优先级为问卷星序号、规范化手机号、规范化微信号。
- 姓名、电话、微信、受光类型若已被 RA 人工校正，后续累计表不会覆盖。
...
今天的记录保持 `pending`，不会提前生成缺失异常。过去日期按缺晚问卷、Lighting 未上传/样本不足/无法解析、缺 GPS、缺次晨问卷分别生成异常。重新运行后，已经补齐的自动异常改为 `resolved`；人工关闭的异常不会被强制重开。
